...
const RECAPTCHA_SITE_KEY = "6LefrcAqAAAAAAtZfDekeyxDXnwy872FU7phEwwc"; // cybervidya KIET site key
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response) {
        ...
        if (statusCode === 400 && errorData?.error?.reason) {
          setError(`Login failed: ${errorData.error.reason}`);
        } else {
          setError(
            `Failed to fetch attendance data (${statusCode}). Please check your credentials.`
          );
        }
      } else {
        setError(
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-8 manga-border manga-fade-in">
            <div className="flex items-center gap-6 mb-6">
              ...
              <div className="flex flex-col">
                <h1 className="text-2xl font-black text-black mb-2 text-center manga-text">
